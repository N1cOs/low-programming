...
read_status read_from_bmp(FILE *in, image *read_in) {
    ...
    read_in->data = (pixel*) ((uint8_t*) read_in->data + BMP_OFFSET);
    return READ_OK;
}
...
image *rotate(image *source_image) {
    ...
    for (row = 0; row < new_image->height; row++) {
        ...
        for (column = 0; column < new_image->width; column++) {
            pixel *px = (void*)(source_image->data + source_image->width * (column + 1) - row - 1) + old_padding * column;
            source_px[column] = *px;
        }
...
int main(int argc, char *argv[]) {
    ...
    FILE *test = fopen(file_name, "r+b");
    image img;
    read_status rd_status = read_from_bmp(test, &img);

    image *new_image = rotate(&img);
    write_to_bmp(test, new_image);
    fclose(test);
    return 0;
}
